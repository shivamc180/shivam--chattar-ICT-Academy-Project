...
from azure.eventhub.aio import EventData
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    try:
        ...
        # Create an EventHubProducerClient object
        producer = EventHubProducerClient.from_connection_string(eventhub_connection_str, eventhub_name)

        # Create a BlobServiceClient object
        blob_service_client = BlobServiceClient.from_connection_string(blob_storage_connection_str)

        # Get user input (until they enter "quit")
        userText = req.get_json()['userText']
        while userText.lower() != 'quit':
            ...
            # Send the result to EventHub
            event_data_batch = producer.create_batch()
            event_data = EventData(body=json.dumps(result))
            event_data_batch.add(event_data)
            producer.send_batch(event_data_batch)

            # Save the result to Blob storage
            blob_client = blob_service_client.get_blob_client(blob_container_name, userText)
